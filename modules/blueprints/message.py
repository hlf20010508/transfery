...
async def page(request):
    ...
    result = await sql.query_items(
        start=start, amount=ITEM_PER_PAGE, access_private=check_login(request)
    )
...
async def sync(request):
    ...
    result = await sql.query_items_after(
        id=latest_id, access_private=check_login(request)
    )
    print("synced:", result)
...
async def new_item(request):
    ...
    await socketio.emit(
        "newItem", item, room=None if not item["isPrivate"] else "private", skip_sid=sid
    )
    print("broadcasted")
