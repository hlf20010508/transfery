...
async def query_is_device_fingerprint_exist(fingerprint):
    sql = 'select count(*) from %s where fingerprint = "%s"' % (MYSQL_TABLE_DEVICE, fingerprint)
...
async def update_device(item):
    await database.table_update(MYSQL_TABLE_DEVICE, item, "fingerprint", item['fingerprint'])
...
async def remove_device(fingerprint):
    await database.query(
        'delete from %s where fingerprint="%s"' % (
            MYSQL_TABLE_DEVICE,
            fingerprint
        )
    )
